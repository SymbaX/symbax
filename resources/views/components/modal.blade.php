...
<div x-data="{
    ...
    focusables() {
        ...
        let selector = 'a, button, input:not([type=\'hidden\']), textarea, select, detail, [tabindex]:not([tabindex=\'-1\'])'
...
})" x-on:open-modal.window="$event.detail == '{{ $name }}' ? show = true : null" x-on:close.stop="show = false" x-on:keydown.escape.window="show = false" x-on:keydown.tab.prevent="$event.shiftKey || nextFocusable().focus()" x-on:keydown.shift.tab.prevent="prevFocusable().focus()" {{ $attributes->has('focusable') ? 'x-ref="modal"' : '' }} x-show="show" class="fixed inset-0 overflow-y-auto px-4 py-6 sm:px-0 z-50" style="display: {{ $show ? 'block' : 'none' }};">
